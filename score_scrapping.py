...
def main_v3():
    # Define the path to the directory containing the results
    results_dir = Path("/data/stympopper/TSPFN-Benchmark/UCRUnivariate").glob("*-TabPFN3-FineTune-NoScheduler")
    ...
    # results_df = filter_datasets_by_class_count(results_df)
    output_dir = "/data/stympopper/TSPFN-Benchmark"
    results_df.to_csv(f"{output_dir}/less_10_ucr_univariate_results_summary-TabPFN3-FineTune-NoScheduler.csv", index=False)
    print("Results summary saved to less_10_ucr_univariate_results_summary-TabPFN3-FineTune-NoScheduler.csv")
