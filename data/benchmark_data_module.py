...
class TSPFNDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        raise NotImplementedError("This is an abstract base class. Please implement setup() in a subclass.")
...
class ECG5000DataModule(TSPFNDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        """Create datasets. Called on every process in distributed settings."""
        self.train_dataset = ECG5000Dataset(root=self.data_roots, split="train")
        self.val_dataset = ECG5000Dataset(root=self.data_roots, split="test")
...
class ECG200DataModule(TSPFNDataModule):
    """LightningDataModule for ECG200 dataset.
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        """Create datasets. Called on every process in distributed settings."""
        self.train_dataset = ECG200Dataset(root=self.data_roots, split="train")
        self.val_dataset = ECG200Dataset(root=self.data_roots, split="test")
...
class ECGFiveDaysDataModule(TSPFNDataModule):
    """LightningDataModule for ECGFiveDays dataset.
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        """Create datasets. Called on every process in distributed settings."""
        self.train_dataset = ECGFiveDaysDataset(root=self.data_roots, split="train")
        self.val_dataset = ECGFiveDaysDataset(root=self.data_roots, split="test")
...
class ORCHIDDataModule(TSPFNDataModule):
    ...
    def val_dataloader(self):
        if not self.mantis_training:
            loaders = {
                "val": self._dataloader(self.val_dataset, shuffle=False, batch_size=self.val_batch_size),
                "train": self._dataloader(self.train_dataset, shuffle=False, batch_size=self.train_batch_size),
            }
            return CombinedLoader(loaders, mode="max_size_cycle")
        else:
