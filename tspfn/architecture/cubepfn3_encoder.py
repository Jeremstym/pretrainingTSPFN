...
class TabPFNV3(Architecture):
    ...
    def __init__(
        ...
        *,
        ...
        self.unified_channel_attention = Attention(
            ...
            num_heads=config.feat_agg_num_heads,  # Use pretrained head topology (e.g., 8)
            head_dim=config.embed_dim // config.feat_agg_num_heads,  # True projection dimension
            **kw,
        )
    ...
    def forward(
        ...
        *,
        ...
        # num_train = train_emb.shape[1] // num_channels
        # num_test = test_emb.shape[1] // num_channels
    ...
    def _process_channel_chunks(
        ...
        """Natively computes graph-safe cross-channel token attention mapping.
        
        Args:
            row_embedding_chunk: Tensor of shape (B, R_chunk, Ch, Cl, E)
            
        Returns:
            Structured row chunk tensor of shape (B, R_chunk, Ch, Cl, E)
        """
        B, R_chunk, Ch, Cl, E = row_embedding_chunk.shape

        # 1. Isolate row chunk layout to safely flatten structural dimensions
        # Shape transitions: (B, R_chunk, Ch, Cl, E) -> (B * R_chunk, Ch * Cl, E)
        flat_sequence = row_embedding_chunk.reshape(B * R_chunk, Ch * Cl, E)
        
        # 2. Process via the properly scaled multi-head module
        attended_flat = self.unified_channel_attention(flat_sequence)
        
        # 3. Restore original target shapes cleanly without layout corruption
        return attended_flat.view(B, R_chunk, Ch, Cl, E)
