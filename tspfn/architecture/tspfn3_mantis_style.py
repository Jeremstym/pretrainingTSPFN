...
class TabPFNV3Cache:
    ...
    train_embeddings: Tensor | None = None
    train_shape: tuple[int, int] = (0, 0)
    scaler_cache: dict[str, Tensor] | None = None
    inducing_hidden: list[Tensor] | None = None
    ...
    def to(self, device: torch.device | str) -> TabPFNV3Cache:
        ...
        return TabPFNV3Cache(
            ...
            scaler_cache=(
                {k: v.to(device) for k, v in self.scaler_cache.items()} if self.scaler_cache is not None else None
            ),
            inducing_hidden=(
                [h.to(device) for h in self.inducing_hidden] if self.inducing_hidden is not None else None
            ),
        )
    ...
    def cache_size_mb(self) -> int:
        ...
        if self.train_embeddings is not None:
            total += self.train_embeddings.numel() * self.train_embeddings.element_size()
...
class CrossAttention(nn.Module):
    ...
    def forward(
        self,
        x_for_query_BQE: Tensor,
        x_for_key_and_value_BVE: Tensor,
    ) -> Tensor:
...
class InducedSelfAttentionBlock(nn.Module):
    ...
    def _induced_attention(
        ...
        x_BcRE: Tensor,
        single_eval_pos: int | None = None,
        cached_hidden: Tensor | None = None,
        *,
...
class FeatureDistributionEmbedder(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        cached_hidden: list[Tensor] | None = None,
        return_hidden: bool = False,
        ...
        """
        hidden_states: list[Tensor] | None = [] if return_hidden else None
...
class ColumnAggregator(nn.Module):
    ...
    def forward(
        ...
        x_full: Tensor = x  # type: ignore[assignment]
...
class TabPFNV3(Architecture):
    ...
    def forward(
        ...
        x_diff: Tensor | dict[str, Tensor] | None,
        ...
        *,
        ...
        test_targets_MB: Tensor | None = None,
    ) -> Tensor | dict[str, Tensor] | tuple[Tensor | dict[str, Tensor], TabPFNV3Cache | None]:
        ...
        if x_diff is not None:
            ...
            print(f"x_RiBAC shape: {x_RiBAC.shape}, std_ts shape: {std_ts.shape}, mean_ts shape: {mean_ts.shape}")
            print(f"x_BRiAClE shape: {x_BRiAClE.shape}, x_BRiAClE_2_emb shape: {x_BRiAClE_2_emb.shape}")
            print(f"x_BRiAClE_2_emb shape: {x_BRiAClE_2_emb.shape}, std_emb shape: {std_emb.shape}, mean_emb shape: {mean_emb.shape}")
            batched_std_emb = std_emb.unsqueeze(0).expand(-1, -1, -1, x_BRiAClE.shape[3], -1)
            batched_mean_emb = mean_emb.unsqueeze(0).expand(-1, -1, -1, x_BRiAClE.shape[3], -1)
        ...
        # ---- Decoder -----------------------------------------------------------
        train_emb = train_emb.view(B, -1, num_channels, self.icl_emsize)
        test_emb = test_emb.view(B, -1, num_channels, self.icl_emsize)

        if self.task_type == "output_embeddings":
            assert (
                train_emb.shape[0] == 1 and test_emb.shape[0] == 1
            ), "Output embeddings only supported for dataset batch size 1."
            embeddings = torch.cat([train_emb, test_emb], dim=1).squeeze(0) # (S, C, E)
            return embeddings
        ...
        if self.task_type == "multiclass":
            ...
            test_out: Tensor = self.many_class_decoder(
                train_emb,
                test_emb,
                y_BN[:, :num_train],
            )
    ...
    def _preprocess_raw(
        ...
        """
        nan_ind_BRiAC: Tensor | None = None
        if self.use_nan_indicators:
    ...
    def _compute_all_inducing_hidden(
        ...
        x_grouped_BRiACG: Tensor,
        num_train: int,
        y_col_emb_BNE: Tensor | None,
        col_chunk_size: int,
        *,
        ...
        hidden_per_block: list[list[Tensor]] = [[] for _ in range(num_blocks)]
    ...
    def _preprocess_and_group(
        ...
        y: Tensor,
        num_train: int,
        scaler_cache: dict[str, Tensor] | None,
    ) -> tuple[Tensor, Tensor | None]:
    ...
    def _stages_0_to_2(
        ...
        *,
        ...
        if kv_cache is not None and not kv_cache.is_empty():
            ...
            precomputed_hidden: list[Tensor] | None = kv_cache.inducing_hidden
    ...
    def _process_row_chunk(
        self,
        x_grouped_chunk_BRjCG: Tensor,
        y_col_emb: Tensor | None,
        chunk_start: int,
        chunk_end: int,
        effective_num_train: int,
        precomputed_hidden: list[Tensor] | None,
...
def _generate_nan_and_inf_indicator(x: Tensor) -> Tensor:
...
def _spline_based_regression_borders(num_buckets: int) -> Tensor:
...
class TSPFNEncoder(nn.Module, ABC):
    ...
    def forward(self, ts: Tensor, ts_diff: Tensor, y: Tensor, **kwargs) -> Tensor:
